              ...
              <div className="flex-1 overflow-y-auto p-6">
                {items.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-primary-dark/60 text-lg">Your cart is empty</p>
                    <p className="text-primary-dark/40 text-sm mt-2">
                  ...
                  <div className="space-y-4">
                    {items.map((item) => (
                      <motion.div
                        ...
                        className="flex gap-4 p-4 bg-primary-pink/10 rounded-xl"
                      >
                        ...
                        <div className="flex-1">
                          ...
                          {item.customMessage && (
                            <p className="text-xs text-primary-rose mt-1 italic">
                              &quot;{item.customMessage}&quot;
                            </p>
                          )}
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center space-x-2">
                              <button
                                ...
                                className="w-6 h-6 rounded-full bg-primary-pink flex items-center justify-center hover:bg-primary-rose hover:text-white transition-colors"
                              >
                                <Minus className="w-3 h-3" />
                              </button>
                              <span className="font-semibold text-primary-dark w-6 text-center">
                                {item.quantity}
                              </span>
                              <button
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
              ...
              {items.length > 0 && (
                <div className="border-t p-6 space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-primary-dark/60">Subtotal</span>
                    <span className="font-semibold text-primary-dark">
                      ₹{getTotalPrice().toFixed(0)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-primary-dark/60">Delivery</span>
                  ...
                  <button
                    onClick={clearCart}
                    className="w-full py-2 text-primary-dark/60 hover:text-red-500 transition-colors text-sm"
                  >
                    Clear Cart
                  </button>
                </div>
